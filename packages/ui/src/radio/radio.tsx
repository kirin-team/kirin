...
function Radio({
  ...
  if (inGroup) {
    if (checked !== undefined)
      useWarning('Remove props "checked" if in a Radio.Group.', 'Radio');

    if (radioValue === undefined)
      useWarning('Props "value" must be defined if in a Radio.Group.', 'Radio');
  ...
  const changeHandler = (event: ChangeEvent) => {
    ...
    if (inGroup) updateState?.(radioValue as string | number);
    onChange?.(selfEvent);
  };
...
export default withScale(Radio);
