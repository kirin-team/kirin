...
function isObjectClassName(value: className): value is classNamesObject {
  return typeof value === 'object' && !Array.isArray(value);
}

export default function useClasses(...classNames: Array<className>) {
  let classes = '';

  const len = classNames.length;
  if (!len) return classes;
  ...
  for (let index = 0; index < len; index++) {
    ...
    if (isObjectClassName(val)) classes += ` ${classObjectToString(val)}`;
    else classes += ` ${String(val).trim()}`;
  }
